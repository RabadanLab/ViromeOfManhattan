...
${d}/scripts/makereport.py $blacklist blast/top.concat >> report/blast.topfilter.txt
...
if [ ${remap} -eq 1 ]; then
	...
	blastdbcmd -db /ifs/scratch/c2b2/rr_lab/shares/ref/blastdb/nt-current/nt -entry_batch report/entry.batch.txt -outfmt '%f' > report/pathogen.ref.fa
	echo runnning STAR genome generate
	mkdir -p report/star_ref report/star_map
	# "[seg fault] is a known problem for very small genomes. At the genome generation step, please try to reduce the value of --genomeSAindexNbases to <=8, and then re-run the mapping step.
	# Generally, --genomeSAindexNbases needs to be scaled with the genome length, as ~min(14,log2(ReferenceLength)/2 - 1). I will need to incorporate this scaling in the future releases." -alexdobin
